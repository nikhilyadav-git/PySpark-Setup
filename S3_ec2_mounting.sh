...
>  sudo s3fs s3-kafka-nemis /nemis-datalake -o iam_role="EC2-BoomiPython-Amazon2-P-DellBoomiETLPythonServer-1NJ8TT6QVQTO8" -o url="https://s3-eu-west-1.amazonaws.com" -o endpoint=eu-west-1 -o dbglevel=info -o curldbg
...
> nano /etc/rc.local
#####Add below line#####
/bin/s3fs s3-kafka-nemis -o use_cache=/tmp -o allow_other -o uid=1001 -o mp_umask=002 -o multireq_max=5 /nemis-datalake
########################

#Check mounted s3 bucket. Output will be similar as shown below but Used size may differ.
> df -Th /nemis-datalake/
